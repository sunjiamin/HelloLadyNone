 ...
 * 类描述：
 * 创建人：Jiamin.Sun
 * 创建时间：3/9/2016 4:22 PM
 * 修改人：Jiamin.Sun
 * 修改时间：3/9/2016 4:22 PM
 * 修改备注：
 */
public class SimplePhotoView extends ImageView {
    ...
    public SimplePhotoView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private Runnable mClickRunnable = new Runnable() {
        ...
        public void run() {
            if (mClickListener != null) {
                mClickListener.onClick(SimplePhotoView.this);
            }
        }
    };
    ...
    public PhotoInfo getInfo() {
        ...
        return new PhotoInfo(rect, local, mImgRect, mWidgetRect, mScale, mDegrees, mScaleType);
    }
